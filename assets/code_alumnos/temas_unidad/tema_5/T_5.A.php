<?php
    ...
    // La columna de calificación se basa en la UNIDAD (calf_A_5)
    ...
    // --- DATOS DEL USUARIO ---
    // Usamos nocontrol para CARPETAS y ARCHIVOS
    $numeroControl = $_SESSION['nocontrol'];
    // Usamos id_usuario para BASE DE DATOS
    $idUsuario = $_SESSION['id_usuario']; // Asegúrate que el login guarde esto en sesión
    
    // Directorio físico (Usa No. Control)
    ...
    try {
        ...
        // 1. Obtener Fechas (Esto es general, no depende del alumno)
        $idUnidadMaestra = 1; // Ojo: ¿Las fechas están en unidad 1 siempre? Si no, cambiar a $idUnidad
        $stmtFechas = $conn->prepare("SELECT $columnaFechaInicial, $columnaFechaFinal FROM alumnos_actividad_fecha WHERE id_unidad = ?");
        $stmtFechas->bind_param("i", $idUnidadMaestra); // Usamos la variable de unidad maestra o actual
        $stmtFechas->execute();
        ...
        if ($fechaInicialDB === null || $fechaFinalDB === null) {
            $actividadAbierta = false;
            $mensajeEstadoActividad = '<div class="alert alert-warning">Esta actividad aún no ha sido configurada por el profesor.</div>';
        } else {
            ...
            if ($fechaActual < $fechaInicial) {
                $actividadAbierta = false;
                $mensajeEstadoActividad = '<div class="alert alert-info">La actividad abrirá el ' . $fechaInicial->format('d/m/Y H:i') . '.</div>';
            } elseif ($fechaActual > $fechaFinal) {
                $actividadAbierta = false;
                $mensajeEstadoActividad = '<div class="alert alert-danger">La actividad cerró el ' . $fechaFinal->format('d/m/Y H:i') . '.</div>';
            } else {
        ...
        $mensajeResultado = '<div class="alert alert-danger">Error DB: ' . $e->getMessage() . '</div>';
    }

    // --- LÓGICA CANCELAR ENTREGA ---
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["cancelarEntrega"])) {
        if (!$actividadAbierta) {
             $_SESSION['mensajeResultado'] = '<div class="alert alert-danger">Actividad cerrada.</div>';
        } elseif ($calificacion > 0) {
             $_SESSION['mensajeResultado'] = '<div class="alert alert-danger">Ya calificada.</div>';
            ...
            try {
                ...
                $stmtCancelBit->close();
                $actividadEntregadaBit = 0;
                $_SESSION['mensajeResultado'] = '<div class="alert alert-success">Entrega cancelada.</div>';
            } catch (mysqli_sql_exception $e) {
                $_SESSION['mensajeResultado'] = '<div class="alert alert-danger">Error: ' . $e->getMessage() . '</div>';
            }
        }
    }

    // --- LÓGICA ELIMINAR ARCHIVO ---
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["eliminarArchivo"])) {
        if (!$actividadAbierta) {
            $_SESSION['mensajeResultado'] = '<div class="alert alert-danger">Actividad cerrada.</div>';
             ...
             $_SESSION['mensajeResultado'] = '<div class="alert alert-danger">Cancele la entrega primero.</div>';
        } elseif ($calificacion > 0) {
             $_SESSION['mensajeResultado'] = '<div class="alert alert-danger">Ya calificada.</div>';
        } elseif (isset($numeroControl)) { // Validamos que exista sesión
            $archivoAEliminar = basename($_POST['nombreArchivo']);
            $rutaCompletaArchivo = $directorioDestino . $archivoAEliminar;

            // Validación de seguridad: que el archivo pertenezca al usuario (nombre contiene nocontrol)
            if (strpos($archivoAEliminar, $numeroControl . '_' . $identificadorActividad . '_') === 0 && file_exists($rutaCompletaArchivo)) {
                if (unlink($rutaCompletaArchivo)) {
                    $_SESSION['mensajeResultado'] = '<div class="alert alert-success">Archivo eliminado.</div>';
                    $archivosSubidos = glob($patronBusqueda);
                    
                    // Si se quedó sin archivos y por error estaba en 1, lo bajamos a 0
                    ...
                    $_SESSION['mensajeResultado'] = '<div class="alert alert-danger">Error al borrar archivo.</div>';
                }
            } else {
                $_SESSION['mensajeResultado'] = '<div class="alert alert-danger">Archivo no válido.</div>';
            }
        }
    }
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["subirArchivo"])) {
        if ($actividadEntregadaBit == 1) { 
            $_SESSION['mensajeResultado'] = '<div class="alert alert-warning">Entrega marcada. Cancele para editar.</div>';
            ...
            if ($calificacion > 0) {
                $_SESSION['mensajeResultado'] = '<div class="alert alert-warning">Ya calificada.</div>';
            } else if ($conteoExistentes >= 5) {
                $_SESSION['mensajeResultado'] = '<div class="alert alert-danger">Límite de 5 archivos alcanzado.</div>';
            } else if (isset($_FILES["archivoActividad"])) {

                // Normalización del array de archivos
                ...
                if ($cantidadAsubir > $slotsDisponibles) {
                    $_SESSION['mensajeResultado'] = '<div class="alert alert-danger">Solo puedes subir ' . $slotsDisponibles . ' más.</div>';
                } else {
                    $resultadosHTML = [];
                    $subidasExitosas = 0;
                    ...
                    for ($i = 0; $i < $cantidadAsubir; $i++) {
                        if ($_FILES["archivoActividad"]["error"][$i] == 0) {
                            ...
                            // Nombre físico usando No. Control (Correcto para sistema de archivos)
                            ...
                            if (file_exists($rutaChequeo)) {
                                ...
                                $resultadosHTML[] = '<div class="alert alert-warning">Omitido: ' . htmlspecialchars($nombreOriginal) . ' > 8MB.</div>';
                            
                            } else {
                                $extensionesPeligrosas = ["php", "exe", "sh", "js"]; // Lista resumida para ejemplo
                                if (in_array($tipoArchivoOriginal, $extensionesPeligrosas)) {
                                    $resultadosHTML[] = '<div class="alert alert-danger">Formato no permitido.</div>';
                                } else {
                                    ...
                                    if (move_uploaded_file($_FILES["archivoActividad"]["tmp_name"][$i], $rutaArchivoCompleta)) {
                                        $resultadosHTML[] = '<div class="alert alert-success">Subido: ' . htmlspecialchars($nombreOriginal) . '</div>';
                                        $subidasExitosas++;
                                    } else {
                    ...
                    // Si subió al menos uno, marcamos como entregado en BD usando ID_USUARIO
                    if ($subidasExitosas > 0) {
                        $archivosSubidos = glob($patronBusqueda);
                        
                        // Insert or Update usando ID_USUARIO
            ...
            $_SESSION['mensajeResultado'] = '<div class="alert alert-danger">Error de sesión.</div>';
        }
    }
...
<div class="contenedor-cursos">
    <div id="mainContent">
        <h2 class="text-center mb-4"><?php echo $textos['parrafo_1012_1']; ?></h2>
        <h3><strong><p class="text-center"><?php echo $textos['parrafo_1012']; ?></p></strong></h3>
                <strong><p class="justificado"><?php echo $textos['parrafo_0874']; ?></p></strong>
        <p class="justificado"><?php echo $textos['parrafo_1013']; ?></p>
        <p class="justificado"><?php echo $textos['parrafo_0876']; ?></p>
        <ul class="list-unstyled justificado mt-3">
            <li class="mb-3">
                <i class="bi bi-arrow-right-circle-fill text-primary me-2"></i><?php echo $textos['parrafo_1014']; ?>
            </li>
            <li class="mb-3">
                <i class="bi bi-arrow-right-circle-fill text-primary me-2"></i><?php echo $textos['parrafo_1015']; ?>
            </li>
            <li class="mb-3">
                <i class="bi bi-arrow-right-circle-fill text-primary me-2"></i><?php echo $textos['parrafo_1016']; ?>
            </li>
            <li class="mb-3">
                <i class="bi bi-arrow-right-circle-fill text-primary me-2"></i><?php echo $textos['parrafo_1017']; ?>
            </li>
        </ul>
        <strong><p class="justificado"><?php echo $textos['parrafo_0851']; ?></p></strong>
        <p class="justificado"><?php echo $textos['parrafo_1018']; ?></p>
        <p class="justificado"><?php echo $textos['parrafo_1019']; ?></p>
        <p class="justificado"><?php echo $textos['parrafo_1020']; ?></p>
        <ul class="list-unstyled justificado mt-3">
            <p class="justificado"><?php echo $textos['parrafo_1021']; ?></p>
            <li class="mb-3">
                <i class="bi bi-arrow-right-circle-fill text-primary me-2"></i><?php echo $textos['parrafo_1022']; ?>
            </li>
            <p class="justificado"><?php echo $textos['parrafo_1023']; ?></p>
            <li class="mb-3">
                <i class="bi bi-arrow-right-circle-fill text-primary me-2"></i><?php echo $textos['parrafo_1024']; ?>
            </li>
            <p class="justificado"><?php echo $textos['parrafo_1025']; ?></p>
            <li class="mb-3">
                <i class="bi bi-arrow-right-circle-fill text-primary me-2"></i><?php echo $textos['parrafo_1026']; ?>
            </li>
            <p class="justificado"><?php echo $textos['parrafo_1027']; ?></p>
            <li class="mb-3">
                <i class="bi bi-arrow-right-circle-fill text-primary me-2"></i><?php echo $textos['parrafo_1028']; ?>
            </li>
            <p class="justificado"><?php echo $textos['parrafo_1029']; ?></p>
            <li class="mb-3">
                <i class="bi bi-arrow-right-circle-fill text-primary me-2"></i><?php echo $textos['parrafo_1030']; ?>
            </li>
        </ul>
        <p class="justificado"><?php echo $textos['parrafo_1031']; ?></p>
        <p class="justificado"><?php echo $textos['parrafo_1032']; ?></p>
        <p class="justificado"><?php echo $textos['parrafo_1033']; ?></p>
        <p class="justificado"><?php echo $textos['parrafo_1034']; ?></p>
        <p class="justificado"><?php echo $textos['parrafo_1035']; ?></p>
        <ul class="list-unstyled justificado mt-3">
            <p class="justificado"><?php echo $textos['parrafo_1036']; ?></p>
            <li class="mb-3">
                <i class="bi bi-arrow-right-circle-fill text-primary me-2"></i><?php echo $textos['parrafo_1037']; ?>
            </li>
            <p class="justificado"><?php echo $textos['parrafo_1038']; ?></p>
            <li class="mb-3">
                <i class="bi bi-arrow-right-circle-fill text-primary me-2"></i><?php echo $textos['parrafo_1039']; ?>
            </li>
            <li class="mb-3">
                <i class="bi bi-arrow-right-circle-fill text-primary me-2"></i><?php echo $textos['parrafo_1040']; ?>
            </li>
            <li class="mb-3">
                <i class="bi bi-arrow-right-circle-fill text-primary me-2"></i><?php echo $textos['parrafo_1041']; ?>
            </li>
            <li class="mb-3">
                <i class="bi bi-arrow-right-circle-fill text-primary me-2"></i><?php echo $textos['parrafo_1042']; ?>
            </li>
        </ul>
        <strong><p class="justificado"><?php echo $textos['parrafo_0857']; ?></p></strong>
        <p class="justificado"><?php echo $textos['parrafo_1043']; ?></p>
        <p class="justificado"><?php echo $textos['parrafo_1044']; ?></p>
        <p class="justificado"><?php echo $textos['parrafo_1045']; ?></p>
        <p class="justificado"><?php echo $textos['parrafo_1046']; ?></p>
        <p class="justificado"><?php echo $textos['parrafo_1047']; ?></p>
        <p class="justificado"><?php echo $textos['parrafo_1048']; ?></p>
        <strong><p class="justificado"><?php echo $textos['parrafo_0865']; ?></p></strong>
        <p class="justificado"><?php echo $textos['parrafo_1049']; ?></p>
        <strong><p class="justificado"><?php echo $textos['parrafo_0867']; ?></p></strong>
        <p class="justificado"><?php echo $textos['parrafo_1050']; ?></p>
        <strong><p class="justificado"><?php echo $textos['parrafo_0869']; ?></p></strong>
        <p class="justificado"><?php echo $textos['parrafo_1051']; ?></p>
        <p class="justificado"><?php echo $textos['parrafo_1052']; ?></p>
        <p class="justificado"><?php echo $textos['parrafo_1053']; ?></p>
        <p class="justificado"><?php echo $textos['parrafo_1054']; ?></p>
        <p class="justificado"><?php echo $textos['parrafo_1055']; ?></p>
        <strong><p class="justificado"><?php echo $textos['parrafo_0841']; ?></p></strong>
        <p class="justificado"><?php echo $textos['parrafo_1056']; ?></p>
        <p class="justificado"><?php echo $textos['parrafo_1057']; ?></p>
        <ul class="list-unstyled justificado mt-3">
            <li class="mb-3">
                <i class="bi bi-arrow-right-circle-fill text-primary me-2"></i><?php echo $textos['parrafo_1058']; ?>
            </li>
            <li class="mb-3">
                <i class="bi bi-arrow-right-circle-fill text-primary me-2"></i><?php echo $textos['parrafo_1059']; ?>
            </li>
            <li class="mb-3">
                <i class="bi bi-arrow-right-circle-fill text-primary me-2"></i><?php echo $textos['parrafo_1060']; ?>
            </li>
            <li class="mb-3">
                <i class="bi bi-arrow-right-circle-fill text-primary me-2"></i><?php echo $textos['parrafo_1061']; ?>
            </li>
        </ul>
