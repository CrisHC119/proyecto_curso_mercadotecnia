<?php
    ...
    // --- CONFIGURACIÓN ESPECÍFICA UNIDAD 5 ---
    $id_examen = 5;        // ID en tabla examen_pregunta
    $id_unidad_actual = 5; // ID en tabla examen_unidad (para fechas)
    
    $puntos_totales = null;
    $examen_enviado = $_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST);
    $zonaHorariaLocal = new DateTimeZone('America/Mexico_City');
    
    // --- 1. OBTENER DATOS DEL ALUMNO (CAMPOS U5) ---
    $sqlTipo = $conn->prepare("SELECT tipo_examen_U5, calf_5, examen_U5, inicio_examen_U5 FROM alumnos_calificacion WHERE id_usuario = ?");
    $sqlTipo->bind_param("i", $id_usuario);
    $sqlTipo->execute();
    $resTipo = $sqlTipo->get_result();
    $row = $resTipo->fetch_assoc();
    
    $examenYaRealizado = ($row && $row['examen_U5'] == 1);
    $inicio_examen_db = $row ? $row['inicio_examen_U5'] : null;
    ...
    if (!$row) {
        // Creamos registro inicial U5
        $in = $conn->prepare("INSERT INTO alumnos_calificacion (id_usuario, tipo_examen_U5) VALUES (?, 1)");
        $in->bind_param("ii", $id_usuario);
        $in->execute();
        $inicio_examen_db = null; 
    } else {
        if ($examenYaRealizado) { 
            $puntos_totales = $row['calf_5'];
        }
    }
    ...
    if ($inicio_examen_db) {
        ...
        $stmt_set_start = $conn->prepare("UPDATE alumnos_calificacion SET inicio_examen_U5 = ? WHERE id_usuario = ? AND inicio_examen_U5 IS NULL");
    ...
    if ($tiempo_restante <= 0 && !$examen_enviado) {
        ...
        $guardar = $conn->prepare("UPDATE alumnos_calificacion SET calf_5 = ?, examen_U5 = 1 WHERE id_usuario = ? AND examen_U5 = 0");
    ...
    if ($examen_enviado && $puntos_totales === null) {
        ...
        $guardar = $conn->prepare("UPDATE alumnos_calificacion SET calf_5 = ?, examen_U5 = 1 WHERE id_usuario = ?");
...
<div class="container py-5">
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <div>
                <h4 class="mb-0 h4"><i class="bi bi-file-earmark-text-fill"></i> Examen Unidad 5</h4>
