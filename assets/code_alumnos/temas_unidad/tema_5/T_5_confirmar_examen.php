<?php
    ...
    $stmt_realizado = $conn->prepare("SELECT examen_U5 FROM alumnos_calificacion WHERE id_usuario = ?");
    $stmt_realizado->bind_param("i", $id_usuario);
    $stmt_realizado->execute();
    $resultado_realizado = $stmt_realizado->get_result();
    $examenRealizado = ($resultado_realizado->fetch_assoc()['examen_U5'] ?? 0) == 1; 
...
<html lang="es">
    <head>
        ...
        <title>Confirmación de Examen - Unidad 5</title> <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
                    ...
                    <div class="card shadow-lg border-0 rounded-4 text-center" style="max-width: 600px; margin: auto;">
                        <div class="card-body p-4 p-sm-5">
                            ...
                            <?php if ($accesoPermitido): ?>
                                ...
                                <p class="text-muted">Estás a punto de iniciar el examen de la <strong>Unidad 5</strong>.</p> <div class="alert alert-info mt-4">
                                ...
                                <form action="T_5_Examen.php" method="post" class="mt-4"> <div class="d-grid gap-2 d-sm-flex justify-content-sm-center">
