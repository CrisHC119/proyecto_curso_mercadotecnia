<?php
    ...
    if ($result_pesos) {
        ...
        error_log("Advertencia: No se encontraron los pesos de calificación. Usando 50/50 por defecto.");
    }
?>
            ...
            <div class="row row-cols-1 row-cols-lg-2 g-4">
                <?php while ($alumno = $result->fetch_assoc()): ?>
                   <?php
                        ...
                        for ($i = 1; $i <= 5; $i++) {
                            $calif_examen = $alumno['calf_' . $i] ?? null;
                            $calif_actividad = $alumno['calf_A_' . $i] ?? null;
                            $pesos = $pesos_unidades[$i] ?? ['examen' => 50, 'actividad' => 50];

                            if ($calif_examen === null && $calif_actividad === null) {
                                $calificaciones_parciales_numericas[$i] = null;
                            } else {
                                ...
                                $calificaciones_parciales_numericas[$i] = round($parcial);
                            }
                        }
                        ...
                        $divisor = 5; 
                        for ($i = 1; $i <= 5; $i++) {
                            $suma_final += ($calificaciones_parciales_numericas[$i] ?? 0); 
                        }
                        $promedio_final = round($suma_final / $divisor); 

                        $calificaciones_para_modal = [];
                        for ($i = 1; $i <= 5; $i++) {
                             $calificaciones_para_modal[$i] = $calificaciones_parciales_numericas[$i] ?? 'N/A';
                        }
                    ?>
                    <div class="col">
                        <div class="card shadow-sm h-100 alumno-card">
                            ...
                            <ul class="list-group list-group-flush">
                                ...
                                <li class="list-group-item d-flex justify-content-between">
                                    <span><i class="bi bi-graph-up me-2 text-body-secondary"></i><?php echo $textos['promedio_final'] ?? 'Promedio Final'; ?></span>
                                    <strong><?php echo $promedio_final; ?></strong>
                                </li>
                            </ul>
                            <div class="card-footer text-end border-0 pt-3">
                                <button type="button" class="btn btn-outline-primary btn-sm"
                                    ...
                                    data-u1="<?php echo htmlspecialchars($calificaciones_para_modal[1]); ?>"
                                    data-u2="<?php echo htmlspecialchars($calificaciones_para_modal[2]); ?>"
                                    data-u3="<?php echo htmlspecialchars($calificaciones_para_modal[3]); ?>"
                                    data-u4="<?php echo htmlspecialchars($calificaciones_para_modal[4]); ?>"
                                    data-u5="<?php echo htmlspecialchars($calificaciones_para_modal[5]); ?>"
                                    data-promedio-final="<?php echo htmlspecialchars($promedio_final); ?>"> <i class="bi bi-person-vcard"></i> <?php echo $textos['ver_detalles']; ?>
                                </button>
...
<script>
    ...
    if (detallesModalEl) {
        const getGradeColorClass = (grade) => {
            if (grade < 70) return 'bg-danger'; 
            if (grade < 90) return 'bg-warning';
            return 'bg-success';                
        };
